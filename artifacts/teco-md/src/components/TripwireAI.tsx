...
export default function TripwireAI() {
  ...
  const handleAdd = () => {
    ...
    toast({ title: ru ? "Добавлено в корзину!" : "Adăugat în coș!", description: product.name });
    setTimeout(() => openCart(), 400);
  };

  const features = ru
    ? ["Tripwire / Зона вторжения", "Распознавание лиц и номеров авто", "Push-уведомление на телефон"]
    : ["Tripwire / Intrusion Zone", "Detectare Față & Plăcuță Auto", "Alertă Push pe Telefon"];

  const featuresCard = ru
    ? ["Tripwire / Intrusion Detection", "Распознавание лиц и номерных знаков", "Анализ трафика людей/транспорта"]
    : ["Tripwire / Intrusion Detection", "Recunoaștere Fețe & Plăcuțe Auto", "Analiză Trafic Persoane/Vehicule"];
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        ...
        <div>
          <span className="inline-block bg-white/10 text-white/80 text-xs font-bold px-3 py-1 rounded-full mb-3">
            {ru ? "МГНОВЕННОЕ ОБНАРУЖЕНИЕ" : "DETECTARE INSTANTĂ"}
          </span>
          <h2 className="font-black text-2xl md:text-3xl mb-1 tracking-tight">AI Tripwire & Analytics</h2>
          <p className="text-zinc-400 text-sm mb-2">
            {ru
              ? "Настраиваемая виртуальная линия. Любое пересечение = мгновенная тревога на телефон — днём и ночью."
          ...
          <div className="flex items-start gap-2 bg-red-950/60 border border-red-800/40 rounded-xl px-3 py-2.5 mb-4 text-xs">
            ...
            <span className="text-red-300">
              {ru
                ? <><strong className="text-red-200">Без ИИ:</strong> камера записывает, но не предупреждает. О краже узнаете только на следующее утро.</>
          ...
          <div className="relative h-[220px] md:h-[300px] rounded-2xl overflow-hidden border border-white/10">
            ...
            <div className="absolute top-3 left-3 bg-black/50 px-2 py-1 rounded-sm flex items-center gap-1.5 z-20">
              ...
              <span className="font-mono text-[10px] font-bold text-red-400">● REC | AI {ru ? "АКТИВЕН" : "ACTIV"}</span>
            </div>
            ...
            <div className="absolute z-10 transition-colors duration-100" style={{ top: "calc(62% - 18px)", left: "10%" }}>
              <span className="font-mono text-[9px] font-bold px-1.5 py-0.5 rounded-sm" style={{
                ...
                {isTriggered
                  ? (ru ? "⚡ СРАБОТАЛО!" : "⚡ TRIPWIRE DECLANȘAT!")
                  : (ru ? "─── ВИРТУАЛЬНАЯ ЛИНИЯ ───" : "─── LINIE VIRTUALĂ ───")
                }
              </span>
            </div>
            ...
            <div className="absolute bottom-3 right-3 bg-black/60 rounded-lg px-2.5 py-1.5 z-20 text-right">
              <div className="font-mono text-xs text-green-400 font-bold">{ru ? "ВХОДЫ:" : "INTRĂRI:"} {count}</div>
              <div className="font-mono text-[9px] text-zinc-500">{ru ? "ТРЕВОГ ОТПРАВЛЕНО:" : "ALERTE TRIMISE:"} {count}</div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl overflow-hidden text-[#09090B]">
          <div className="relative h-44 bg-zinc-100 overflow-hidden">
            ...
            <span className="absolute top-3 right-3 bg-zinc-900 text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
              {ru ? "ИСКУССТВЕННЫЙ ИНТЕЛЛЕКТ" : "INTELIGENȚĂ ARTIFICIALĂ"}
            </span>
          </div>

          <div className="p-5">
            <p className="text-xs text-zinc-400 font-semibold mb-1">
              {ru ? "Рекомендуемый AI-видеорегистратор" : "Înregistrator Video AI Recomandat"}
            </p>
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl px-3 py-2.5 mb-4 text-xs">
              <p className="text-amber-700 font-semibold mb-0.5">💡 {ru ? "Посчитайте стоимость" : "Calculează costul"}</p>
              <p className="text-amber-600">
                {ru
                  ? <>Средний ущерб от кражи — <strong>15.000–80.000 MDL</strong>. Система AI стоит в разы меньше — и оповещает в реальном времени.</>
                  : <>O efracție medie costă <strong>15.000–80.000 MDL</strong>. Sistemul AI costă o fracție din asta — și te alertează în timp real.</>
                }
              </p>
            </div>
            ...
            <div className="flex items-end justify-between mb-4">
              ...
              <span className="text-[10px] text-zinc-400">+{ru ? "НДС включён" : "TVA inclus"}</span>
            </div>
            ...
            <button onClick={handleAdd} data-testid="tripwire-add-btn"
              ...
              <ShoppingCart className="w-4 h-4" /> {ru ? "Хочу AI-систему →" : "Vreau sistemul AI →"}
            </button>
          </div>
        </div>
      </div>
