...
import { X, Check, ChevronDown, ChevronUp, Cookie } from "lucide-react";
...
export function CookieConsent() {
  ...
  const categories = [
    {
      id: "essential",
      title: ru ? "Необходимые" : "Esențiale",
      desc: ru
        ? "Необходимы для работы сайта: корзина, язык, сессия. Не могут быть отключены."
        : "Necesare pentru funcționare: coș, limbă, sesiune. Nu pot fi dezactivate.",
      always: true,
    },
    {
      id: "analytics",
      title: "Analytics",
      desc: ru
        ? "Помогают улучшить сайт. Анонимизированные данные (Google Analytics)."
        : "Ne ajută să îmbunătățim site-ul. Date anonimizate (Google Analytics).",
      value: analytics,
    ...
    },
    {
      ...
      title: "Marketing",
      desc: ru
        ? "Релевантная реклама на других платформах (Facebook Pixel)."
        : "Reclame relevante pe alte platforme (Facebook Pixel).",
      value: marketing,
      ...
      <div className="fixed bottom-[64px] md:bottom-4 left-3 right-3 md:left-auto md:right-4 md:w-auto z-[9999]
        ...
        <div className="bg-zinc-900/95 backdrop-blur-sm text-white rounded-2xl shadow-2xl
          ...
          <p className="text-xs text-zinc-300 flex-1 min-w-0 leading-tight">
            {ru ? "Мы используем cookie-файлы." : "Folosim cookie-uri."}{" "}
            <button
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            >
              <Check className="w-3 h-3" /> {ru ? "Принять" : "Accept"}
            </button>
          </div>
          ...
          <button
            ...
            aria-label={ru ? "Принять необходимые и закрыть" : "Acceptă esențiale și închide"}
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="relative w-full md:w-96 bg-white rounded-t-3xl md:rounded-3xl shadow-2xl
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-zinc-100 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-black text-zinc-900 text-sm">
              {ru ? "Настройки cookie" : "Preferințe Cookie"}
            </span>
          </div>
          <button
        ...
        <div className="overflow-y-auto flex-1 px-4 py-3 space-y-2">
          <p className="text-[11px] text-zinc-400 leading-relaxed px-1 pb-1">
            {ru
              ? <>Используя сайт TECO.MD, вы соглашаетесь с{" "}
                  <a href="/confidentialitate" className="text-[#FF4F00] underline underline-offset-1">Политикой конфиденциальности</a>
                  {" "}и{" "}
                  <a href="/termeni" className="text-[#FF4F00] underline underline-offset-1">Условиями использования</a>.</>
            ...
            <div key={cat.id} className="border border-zinc-100 rounded-2xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  {cat.always ? (
                    <span className="text-[9px] font-black bg-green-100 text-green-700 px-2 py-0.5 rounded-full tracking-wide">
                      {ru ? "ОБЯЗ." : "NECESAR"}
                    </span>
                  ) : (
                    <button
...
export function ReopenConsentButton() {
  ...
  return (
    ...
    >
      {lang === "ru" ? "Настройки cookie" : "Preferințe Cookie"}
    </button>
  );
}
