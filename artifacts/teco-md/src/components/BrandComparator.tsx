...
const BRAND_VISUAL: Record<Brand, {
  ...
  overlay?: string; quality: string; risk: string; riskColor: string;
}> = {
  TAPO: {
    ...
    risk: "Față neidentificabilă pe întuneric", riskColor: "#ef4444",
  },
  REOLINK: {
    ...
    quality: "Bună",
    risk: "Identificare parțială — fără plăcuță auto", riskColor: "#f59e0b",
  },
  UNIARCH: {
    ...
    quality: "Foarte bună",
    risk: "Detalii clare — stocare -50% față de alte branduri", riskColor: "#34d399",
  },
  DAHUA: {
    ...
    quality: "Excelentă (AI)",
    risk: "AI detectează persoane & vehicule — zero alarme false", riskColor: "#4ade80",
  },
  UNIVIEW: {
    ...
    quality: "Premium (Color Noapte)",
    risk: "Color clar noaptea — vezi chipul, nu doar silueta", riskColor: "#fb923c",
  },
  TIANDY: {
    ...
    risk: "Detectare la lumina stelelor — interior și exterior", riskColor: "#67e8f9",
  },
};
...
const BRAND_BADGE: Partial<Record<Brand, string>> = {
  DAHUA: "CEL MAI ALES",
  UNIVIEW: "COLOR NOAPTE",
  TIANDY: "ULTRA PREMIUM",
};

const FALLBACK: Record<Brand, { id: number; name: string; price: number; specs: string; badge: string | null; desc: string; urgency: string }> = {
  TAPO:    { id: 301, name: "Camera Tapo C310 WiFi 2K",          price: 810,  specs: "2K | WiFi | IR 30m",               badge: "BUGET",      desc: "Soluție simplă și accesibilă pentru acasă.",          urgency: "Disponibil azi" },
  REOLINK: { id: 302, name: "Reolink Argus Eco 3MP Battery",     price: 1510, specs: "3MP | Acumulator | WiFi | IP65",   badge: null,         desc: "Fără cabluri — se montează oriunde!",                  urgency: "Livrare 24h" },
  UNIARCH: { id: 303, name: "Uniarch Uho-S2E-M4 4MP WiFi",       price: 940,  specs: "4MP | WiFi | IR 10m | Zoom 2×",   badge: "RECOMANDAT", desc: "Raport excelent preț/calitate.",                       urgency: "Cel mai vândut" },
  DAHUA:   { id: 304, name: "Camera Dahua WizSense 4MP",         price: 1890, specs: "4MP | AI SMD+ | IR 40m | TiOC",   badge: "POPULAR",    desc: "Detectare AI precisă — zero alarme false.",            urgency: "3 bucăți rămase" },
  UNIVIEW: { id: 305, name: "UNV IPC3612LB ColorHunter 2MP",     price: 900,  specs: "2MP | ColorHunter 0.005Lux | PoE",badge: "BEST SELLER",desc: "Color noapte la 0.005 Lux.",                           urgency: "Stoc limitat" },
  TIANDY:  { id: 306, name: "Tiandy TC-C34XN 4MP S+265",         price: 990,  specs: "4MP | S+265 | IR 30m | IP66",     badge: "PREMIUM",    desc: "Robustă și fiabilă la condiții extreme.",              urgency: "Instalare gratuită" },
};
...
export default function BrandComparator() {
  ...
  const handleAdd = () => {
    if (adminProd) {
      ...
      addItem({ ...fb, icon: "Camera", category: "camere", imageUrl: feedImage, model: "", brand: active, description: fb.desc, inStock: true } as any);
  ...
  const brandBadge = BRAND_BADGE[active];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
            <span className="inline-block bg-zinc-100 text-zinc-600 text-xs font-bold px-3 py-1 rounded-full">COMPARĂ BRANDURILE</span>
            <div className="flex items-center gap-1.5 text-xs text-zinc-500">
              <TrendingUp className="w-3.5 h-3.5 text-[#FF4F00]" />
              <span><strong className="text-[#09090B]">847</strong> clienți au ales camera după comparație</span>
            </div>
          </div>
          ...
          <div key={active} className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden flex flex-col" style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <button onClick={handleAdd} data-testid={`comparator-add-${active}`} className="w-full bg-[#FF4F00] text-white font-bold py-3 rounded-xl hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-2 text-sm shadow-[0_4px_14px_rgba(255,79,0,0.25)]">
                <ShoppingCart className="w-4 h-4" /> Vreau această cameră →
              </button>
            </div>
          </div>
        </div>
