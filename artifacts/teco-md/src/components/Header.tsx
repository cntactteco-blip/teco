    ...
    <>
      ...
      {configuratorOpen && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setConfiguratorOpen(false); }}
        >
          <div className="flex-1 overflow-y-auto">
            <Suspense fallback={
              ...
              <BundleBuilder onClose={() => setConfiguratorOpen(false)} />
            </Suspense>
          </div>
        </div>
      )}
        ...
        <div className="flex w-full max-w-7xl mx-auto items-center gap-3 md:gap-4">
          ...
          <div className="flex-shrink-0 flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="hidden md:flex items-center gap-1.5 px-3 py-2 rounded-full bg-zinc-50 hover:bg-[#FF4F00] hover:text-white transition-all text-xs font-semibold text-zinc-700 group"
            >
            ...
            <Link
              href="/favorit"
              className="relative flex items-center justify-center w-10 h-10 rounded-full bg-[#FAFAFA] hover:bg-red-50 transition-colors"
              title="Lista de dorințe"
            >
              <Heart className={`w-5 h-5 transition-colors ${wishCount > 0 ? "text-red-500 fill-red-500" : "text-zinc-500"}`} />
              {wishCount > 0 && (
                <span className="absolute -top-1 -right-1 flex h-4.5 w-4.5 min-w-[18px] items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white border-2 border-white px-0.5">
                  {wishCount}
                </span>
              )}
            </Link>
