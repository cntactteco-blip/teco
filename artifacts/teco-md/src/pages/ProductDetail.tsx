...
export default function ProductDetail() {
  ...
  const handleWhatsApp = () => {
    ...
    const msg = encodeURIComponent(
      `${!product.inStock ? (lang === "ru" ? "Здравствуйте, подскажите, пожалуйста, наличие:" : "Bună ziua, vă rog să-mi confirmați disponibilitatea:") : t("pd.wa_msg")}\n*${product.name}*\nModel: ${product.model}\nMDL: ${product.price.toLocaleString()}\n${t("pd.wa_qty")}: ${qty}`
    );
        ...
        <div className="md:grid md:grid-cols-12 md:gap-10">
          ...
          <div className="md:col-span-6 lg:col-span-5 px-4 md:px-0 pt-4 md:pt-0 flex flex-col gap-0">
            ...
            <div className="flex items-center gap-2.5 mb-2.5">
              ...
              <button ref={addBtnRef} onClick={handleAdd} disabled={!product.inStock}
                className={`relative overflow-hidden flex-1 h-11 rounded-xl font-bold text-sm transition-all active:scale-95 flex items-center justify-center gap-2 ${
                  !product.inStock ? "bg-zinc-200 text-zinc-500 cursor-not-allowed" : added ? "bg-[#10B981] text-white" : "bg-[#FF4F00] text-white hover:opacity-90"
                }`}>
                {!added && <span className="btn-shine-sweep" />}
                {!product.inStock
                  ? (lang === "ru" ? "Нет в наличии" : "Stoc indisponibil")
                  : added
            ...
            <button onClick={handleWhatsApp}
              ...
              {!product.inStock ? (lang === "ru" ? "Уточнить наличие в WhatsApp" : "Întreabă disponibilitatea pe WhatsApp") : t("pd.order_wa")}
            </button>
...
const pct = reviewCount ? (userReviews.filter(review => review.rating === s).length / reviewCount) * 100 : 0;
      ...
      <div className="md:hidden fixed bottom-[56px] left-0 right-0 bg-white/95 backdrop-blur-md border-t border-[#E4E4E7] px-4 py-2.5 z-40 flex items-center gap-3 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        ...
        <button onClick={handleWhatsApp}
          ...
          <Phone className="w-4 h-4" /> {!product.inStock ? (lang === "ru" ? "Уточнить" : "Verifică stocul") : t("pd.order_wa")}
        </button>
        <button onClick={handleAdd} disabled={!product.inStock}
          className={`flex-shrink-0 font-bold px-4 py-2.5 rounded-xl text-sm flex items-center gap-1.5 active:scale-95 transition-all ${
            !product.inStock ? "bg-zinc-200 text-zinc-500 cursor-not-allowed" : added ? "bg-[#10B981] text-white" : "bg-[#FF4F00] text-white"
          }`}>
          {product.inStock && (added ? <Check className="w-4 h-4" /> : <ShoppingCart className="w-4 h-4" />)}
          {!product.inStock ? (lang === "ru" ? "Нет в наличии" : "Indisponibil") : added ? t("pd.added") : t("nav.cart")}
        </button>
      </div>
    </main>
    </>
